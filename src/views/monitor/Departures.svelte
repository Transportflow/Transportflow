...
<div class="absolute w-full p-4 pt-12 overflow-hidden sm:p-24 sm:pl-40 sm:pt-20" transition:fade="{{ duration: 100 }}">
    <div class="sm:max-w-lg">
        <BackButton backTo="/monitor">
            <Button onClick={loadDepartures}
                ...
                <div class="flex">
                    <ion-icon style="zoom: 1;" class="my-auto mr-1 -ml-1 text-gray-900 dark:text-gray-400"
        ...
        {#if monitor === null}
            <div class="flex items-center justify-center h-64">
                <div class="text-center text-gray-700 dark:text-gray-300">
                    <p class="-mb-2 text-4xl"><ion-icon name="alert-circle"></ion-icon></p>
                    <p>{$_("departures.no_departures")}</p>
                </div>
            </div>
        {/if}
        <div class="flex flex-no-wrap mb-2 overflow-scroll overflow-y-hidden scrolling-touch rounded sm:overflow-x-hidden sm:flex-wrap scrollbar-none">
            {#if monitor !== null && monitor.stopovers != undefined}
                <Button style="display: flex; align-items: center; height: 37px" onClick={() => {showMap = !showMap;}}
                        className="{showMap ? 'dark:bg-gray-900 bg-gray-300' : ''} mr-2 sm:my-1 bg-gray-200 dark:bg-gray-700 text-gray-700 dark:text-gray-400 shadow-none">
            ...
            {#if allModes.length > 1 && !showMap}
                {#each allModes as mode (mode.title + mode.img)}
                    <Button onClick={toggleMode.bind(null, mode)}
                            className="{activeModes.indexOf(mode) !== -1 ? 'dark:bg-gray-900 bg-gray-300' : ''} relative flex mr-2 sm:my-1 bg-gray-200 dark:bg-gray-700 text-gray-700 dark:text-gray-400 shadow-none">
                        <span class="ml-6 whitespace-no-wrap">{mode.title}</span>
                        <img class="absolute w-5 h-5 mr-1" alt="" src="{mode.img}"/>
                    </Button>
                {/each}
            {/if}
        </div>
        <div style="max-height: 75vh;"
             class="pb-56 overflow-scroll overflow-x-hidden scrolling-touch rounded-lg scrollbar-none">
            {#if showMap}
