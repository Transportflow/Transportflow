    ...
    <div class="h-full flex justify-center items-center">
        <div class="pb-32">
            {#if !regionName}
                ...
                <p class="text-white text-center">{$_('onboarding.slogan_1')}<br/>{$_('onboarding.slogan_2')}</p>
            ...
            {:else}
                <div class="text-center px-auto mt-16">
                    <ion-icon class="text-white w-12 h-12" name="checkmark-circle"></ion-icon>
                </div>
                <h1 class="text-center text-4xl text-white font-bold">{$_('onboarding.done')}</h1>
                <p class="text-white text-center">{$_('onboarding.setup_done')}</p>
                <Link to="/">
                    <button style="background: #0074D9; border-radius: 0.75rem;"
                            class="primary-button mt-4 w-full text-white py-2 px-auto hover:shadow-lg transition duration-200">
                        {$_('onboarding.lets_go')} ->
                    </button>
                </Link>
                <p class="text-gray-100 opacity-75 text-sm mt-4 w-56 mx-auto text-center">{$_('onboarding.privacy_consent')}</p>
            {/if}
        </div>
    </div>
    <Link to="impressprivacy">
        <p class="text-gray-200 opacity-50 hover:opacity-100 absolute bottom-0 text-center w-full mb-4 transition duration-1000">{$_('utility.impress_privacy')}</p>
    </Link>
